...
def photom_plots(data_fits, master_folder, singular=None, **kwargs):
    ...
    if kwargs["debug"]:
        ...
        plt.scatter(
            ...
            label="archi; Cv = {}".format(round(cv, 2)),
            s=point_size,
        )
        ...
        fig_1.savefig(os.path.join(master_folder, "Star_0/archi_official_compare"))
    ...
    if kwargs["show_results"]:
        ...
        for j in range(len(data_fits.stars)):
            ...
            plt.legend(loc=1)
            plt.savefig(os.path.join(master_folder, "Star_{}/archi_curve".format(j)))
